...
- 🚨 CARO & STATUTORY AUDITOR ANNEXURE EXCLUSION FOR CONSOLIDATED SCOPE: Under Indian Company Law (Companies (Auditor's Report) Order / CARO 2020 Clause xvii, Section 143(11)), statutory auditor annexures and certifications—such as 'Cash Loss incurrence certification' (CARO Clause xvii), 'Working Capital security limits' (CARO Clause ii), or 'Statutory Dues compliance' (CARO Clause vii)—apply STRICTLY AND EXCLUSIVELY to the Standalone parent legal entity! CARO does NOT legally apply to Consolidated group financial statements. Therefore, during this CONSOLIDATED evaluation pass, if any candidate for 'Cash Loss' or 'Cash Loss Incurrence Status' originates from a CARO Clause (xvii) auditor report or standalone statutory annexure, you MUST IMMEDIATELY REJECT IT as out-of-scope for Consolidated group disclosures! Return `final_value: null` for Consolidated Cash Loss unless the group explicitly publishes a formal group-level cash loss statement.
- Record rejections in `rejection_audit_log`: e.g., "[REJECTED Candidate on Page X]: Rejected Segment Report / CARO Standalone annexure figure; out of scope for Consolidated group performance."""
...
- 🚨 CONSOLIDATED VS. STANDALONE CELL VERIFICATION (REIT / TRUST PARITY RULE): When evaluating Consolidated disclosures (especially in REITs, InvITs, and multi-entity trusts where Trust-level distributions or Net Distributable Cash Flow / NDCF are reported across both scopes), DO NOT blindly copy or assume that the Consolidated number is identical to the Standalone number! Even if Standalone and Consolidated figures differ by only 0.01M due to rounding across group SPV inputs (`10,625.79 vs 10,625.80`), you MUST verify and extract the exact numerical digits verbatim from the Consolidated Financial Statements / Notes table (`page_number`) during this Consolidated pass.
...
- 🚨 TABLE HEADER VS. NARRATIVE CHAPTER BOUNDARY FIREWALL: Be extremely vigilant when evaluating tables embedded within management narrative chapters (such as `Key Financial Performance Ratios` tables inside the Directors' Report or MD&A). Even if the surrounding chapter narrative discusses Standalone parent operations, summary tables embedded on those pages frequently present CONSOLIDATED group figures (`Consolidated ROE`, `Consolidated EBITDA Margin`, `Group Net Debt`). You must inspect the table's structural header and audited statement benchmarks; if the ratio table reflects group/consolidated performance, immediately reject it during this Standalone pass!
...
- 🚨 GENERAL STATUTORY AUDITED STATEMENTS PRECEDENCE: Primary Audited Financial Statements (`AUDITED_TABLE`: P&L, Balance Sheet, Cash Flow) and formal Notes (`NOTES_TABLE`) carry the auditor's primary True & Fair View statutory assurance under ICAI SA 700 / ISA 700. They hold ABSOLUTE PRECEDENCE over narrative chapters (`Directors' Report`, `MD&A`), infographic KPI boxes, and voluntary investor highlights. Whenever a candidate found in an `AUDITED_TABLE` or formal `NOTES_TABLE` / `MD&A_TABLE` conflicts with a candidate found in a front-of-report `NARRATIVE_PARAGRAPH` or `KPI_HIGHLIGHTS_BOX`, you MUST ALWAYS reject the narrative highlight and select the formal statement/table candidate! Front-of-report management summaries frequently conflate Consolidated figures or sub-slice KPIs into narrative highlights.
